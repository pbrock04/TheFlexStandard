...
function applyGlobalCentering(source) {
  if (source.includes('id="flex-global-center"')) return source;
  const style = `<style id="flex-global-center">body,main,section,article,aside,footer,form,fieldset,h1,h2,h3,h4,h5,h6,p,small,strong,span,label,legend,a,button,input,select,textarea,.card,.day,.notice,.progress,.completion{text-align:center!important}input::placeholder,textarea::placeholder{text-align:center!important}.day-top{justify-content:center!important;gap:.8rem}</style>`;
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/challenge' || path === '/challenges/7-day') {
      source = enhanceSevenDaySignup(source);
    }
    source = applyGlobalCentering(source);
