...
export async function participantRoute(request, env, path) {
  ...
  try {
    if (request.method === 'POST' && path === '/api/participants/start') {
      ...
      const participant = await createOrResumeParticipant(env.DB, { name: body.name, email: body.email, lead_source: body.lead_source || 'website', progress_snapshot: body.progress_snapshot || null });
      return json({ ok: true, participant }, participant.is_new ? 201 : 200);
    }
    if (request.method === 'POST' && path === '/api/participants/progress') {
      const body = await readJson(request);
      if (body == null) return json({ ok: false, error: 'Invalid JSON request.' }, 400);
      const progress = await recordChallengeProgress(env.DB, { user_id: body.user_id, tier_id: body.tier_id, current_day: body.current_day });
