...
export async function recordLifecycleEvent(db, input) {
  ...
  await db.prepare(`INSERT OR IGNORE INTO lifecycle_events
    (event_id,event_name,user_id,tier_id,lead_source,occurred_at,metadata_json,created_at)
    VALUES (?,?,?,?,?,?,?,?)`)
    .bind(event.event_id, event.event_name, event.user_id, event.tier_id, event.lead_source, event.occurred_at, metadataJson, Date.now())
    .run();
  return event;
}
...
export async function createOrResumeParticipant(db, input = {}) {
  ...
  await db.prepare(`INSERT INTO participants (user_id,email,name,lead_source,created_at,updated_at)
    VALUES (?,?,?,?,?,?)
    ON CONFLICT(email) DO UPDATE SET
      name=COALESCE(excluded.name,participants.name),
      lead_source=participants.lead_source,
      updated_at=excluded.updated_at`)
    .bind(userId, email, name, leadSource, now, now).run();

  await db.prepare(`INSERT INTO participant_tiers
    (user_id,tier_id,status,current_day,unlocked_at,completed_at,updated_at)
    VALUES (?,?,'unlocked',0,?,NULL,?)
    ON CONFLICT(user_id,tier_id) DO NOTHING`)
    .bind(userId, FLEX_TIERS.FOUNDATION, now, now).run();

  if (!existing) {
    await recordLifecycleEvent(db, {
      event_id: `lead_captured:${userId}`,
  ...
  return { user_id: userId, email, name: name || existing?.name || null, foundation_unlocked: true, is_new: !existing };
}
...
export async function recordChallengeProgress(db, { user_id, tier_id, current_day }) {
  ...
  await db.prepare(`UPDATE participant_tiers SET current_day=?,updated_at=? WHERE user_id=? AND tier_id=?`)
    .bind(effectiveDay, now, userId, tier_id).run();

  if (tier_id === FLEX_TIERS.FOUNDATION && effectiveDay >= 3) {
    await recordLifecycleEvent(db, {
      event_id: `foundation_day_3_reached:${userId}`,
  ...
  if (effectiveDay >= completionDay) {
    await db.prepare(`UPDATE participant_tiers SET status='completed',completed_at=COALESCE(completed_at,?),updated_at=? WHERE user_id=? AND tier_id=?`)
      .bind(now, now, userId, tier_id).run();
    await recordLifecycleEvent(db, {
      event_id: `${completionEvent}:${userId}`,
      event_name: completionEvent,
      user_id: userId,
      tier_id,
      occurred_at: now,
    });
    ...
    if (nextTier) {
      await db.prepare(`INSERT INTO participant_tiers
        ...
        ON CONFLICT(user_id,tier_id) DO UPDATE SET
          status=CASE WHEN participant_tiers.status='completed' THEN 'completed' ELSE 'unlocked' END,
          unlocked_at=COALESCE(participant_tiers.unlocked_at,excluded.unlocked_at),
          updated_at=excluded.updated_at`)
